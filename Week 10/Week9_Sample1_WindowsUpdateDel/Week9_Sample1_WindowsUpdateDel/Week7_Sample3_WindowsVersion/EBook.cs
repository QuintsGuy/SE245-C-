...
namespace Week_6_Sample1_DataValidation
{
    class EBook: Book   //Welcome to inheritance...Ebook has all that book has, but more!
    {
        ...
        public string UpdateARecord()
        {
            ...
            string strSQL = "UPDATE EBooks SET Title = @Title, AuthorFirst = @AuthorFirst, AuthorLast = @AuthorLast, Email=@Email, Pages=@Pages, " +
                "DatePublished=@DatePublished, DateRentalExpires=@DateRentalExpires, BookmarkPage=@BookmarkPage  WHERE EBook_ID = @EBook_ID;";
        ...
        {
            return 
                @"Server=sql.neit.edu\studentSqlServer,4500;Database=SE245_SLambert;User Id=SLambert;Password=NunUsBiz;";
        }
